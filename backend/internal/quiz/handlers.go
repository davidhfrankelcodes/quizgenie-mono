...
func CreateQuizHandler(w http.ResponseWriter, r *http.Request) {
  ...
  // assume queueClient was initialized in init()
...
func GetQuizStatusHandler(w http.ResponseWriter, r *http.Request) {
  ...
  // (Optionally, verify qrec.BucketID belongs to the requesting user)
...
func GetQuizQuestionsHandler(w http.ResponseWriter, r *http.Request) {
  ...
  type answerResp struct {
    ...
    IsCorrect   *bool  `json:"isCorrect,omitempty"`   // omit in normal mode
    Explanation *string `json:"explanation,omitempty"` // omit in normal mode
  }
  ...
  for _, q := range questions {
    ...
    for _, a := range ans {
      // In practiceMode, expose correct/explanation; otherwise omit
...
func SubmitQuizHandler(w http.ResponseWriter, r *http.Request) {
  ...
  // 2) For each answer in payload, check correctness and insert AttemptAnswer
  correctCount := 0
...
func ListAttemptsHandler(w http.ResponseWriter, r *http.Request) {
  ...
  // (Optionally verify bucket belongs to user)
...
func GetAttemptDetailsHandler(w http.ResponseWriter, r *http.Request) {
  ...
  // Fetch questions & attemptAnswers
